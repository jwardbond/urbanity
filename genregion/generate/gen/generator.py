...
class RegionGenerator(object):
    ...
    def __init__(self, segs):
        ...
        for seg in segs:
            start = self.node_dict.find(
                seg.start.askey(), Node(seg.start.x, seg.start.y)
            )
            end = self.node_dict.find(seg.end.askey(), Node(seg.end.x, seg.end.y))
            if (
                start == end
            ):  # When the start node and the end node of the segment are very close to each other.
                continue
                    ...
                    if region.area() > 0:
                        if mygeo.is_counter_clockwise(sub_nodes):
                            if points is None:
                                ...
                                raise error.RegionError(
                                    "Too many rings are counter clockwise"
                                )
                        else:
